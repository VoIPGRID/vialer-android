...
public class IconHelper {
    ...
    public static Bitmap getCallerIconBitmap(Context context, String string, String number, int color) {
        if (color == 0) {
        ...
        if (string == null || string.isEmpty()) {
            return addPlaceholderPicture(context, bitmap, color);
        }

        return bitmap;
    }

    private static Bitmap addPlaceholderPicture(Context context, Bitmap circle, int color) {
        Drawable iconDrawable = context.getResources().getDrawable(R.drawable.ic_person);
