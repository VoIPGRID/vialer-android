...
public class CallPresenter {
    ...
    private void updateAudioSourceButton() {
        ...
        if(mActivity.isOnSpeaker()) {
            ...
            mActivity.mSpeakerLabel.setTextColor(ContextCompat.getColor(mActivity, R.color.color_primary));
        } else {
            mActivity.mSpeakerButton.deactivate();
            mActivity.mSpeakerLabel.setTextColor(ContextCompat.getColor(mActivity, R.color.dial_button_chars_color));
        }
