...
import nl.voipgrid.vialer_voip.core.call.Metadata
import nl.voipgrid.vialer_voip.core.call.State
import nl.voipgrid.vialer_voip.providers.pjsip.core.PjsipCall.MicrophoneStatus.*
...
internal abstract class PjsipCall : org.pjsip.pjsua2.Call, Call {
    ...
    private fun updateState() {
        val telephonyState = when(info.state) {
        ...
        telephonyState?.let {
            if (state.telephonyState != it) {
                Log.e("TEST123", "Update state ${info.state} broadcasting $it")
                state.stateHistory.add(it)
