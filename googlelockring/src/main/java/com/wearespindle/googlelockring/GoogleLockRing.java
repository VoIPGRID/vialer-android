...
public class GoogleLockRing extends View {
    private static final String TAG = "GoogleLockRing";
    ...
    public GoogleLockRing(Context context) {
        this(context, null);
    }

    public GoogleLockRing(Context context, AttributeSet attrs) {
        super(context, attrs);

        Resources res = context.getResources();

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.GoogleLockRing);
        mInnerRadius = a.getDimension(R.styleable.GoogleLockRing_innerRadius, mInnerRadius);
        mOuterRadius = a.getDimension(R.styleable.GoogleLockRing_outerRadius, mOuterRadius);
        mSnapMargin = a.getDimension(R.styleable.GoogleLockRing_snapMargin, mSnapMargin);
        mVibrationDuration = a.getInt(R.styleable.GoogleLockRing_vibrationDuration,
                mVibrationDuration);
        mFeedbackCount = a.getInt(R.styleable.GoogleLockRing_feedbackCount,
                mFeedbackCount);
        TypedValue handle = a.peekValue(R.styleable.GoogleLockRing_handleDrawable);
        mHandleDrawable = new TargetDrawable(res, handle != null ? handle.resourceId : 0, 2);
        mHandleDrawable.setState(TargetDrawable.STATE_INACTIVE);
        mOuterRing = new TargetDrawable(
                res,
                getResourceId(a,
                R.styleable.GoogleLockRing_outerRingDrawable),
                1
        );

        mAlwaysTrackFinger = a.getBoolean(R.styleable.GoogleLockRing_alwaysTrackFinger, false);

        int pointId = getResourceId(a, R.styleable.GoogleLockRing_pointDrawable);
        Drawable pointDrawable = pointId != 0 ? res.getDrawable(pointId) : null;
        mGlowRadius = a.getDimension(R.styleable.GoogleLockRing_glowRadius, 0.0f);

        TypedValue outValue = new TypedValue();

        // Read array of target drawables.
        if (a.getValue(R.styleable.GoogleLockRing_targetDrawables, outValue)) {
        ...
        if (a.getValue(R.styleable.GoogleLockRing_targetDescriptions, outValue)) {
        ...
        if (a.getValue(R.styleable.GoogleLockRing_directionDescriptions, outValue)) {
            final int resourceId = outValue.resourceId;
            if (resourceId == 0) {
        ...
        mGravity = a.getInt(R.styleable.GoogleLockRing_android_gravity, Gravity.TOP);
        a.recycle();
